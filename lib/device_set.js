...
function DeviceSetModule(config, log, homebridge, vivintApi) {
  ...
  let config_cameraAutomationHttpPort = config.cameraAutomationHttpPort || null
  ...
  extend(DeviceSet, {
    createDeviceAccessory: (data) => {
      ...
      if (irrelevantDeviceTypes.indexOf(data.Type) != -1) {
        log.debug(`Ignored unuseable device [Type]:${data.Type} [Data]:`, JSON.stringify(data))
        return null
      }

      if (!deviceClass) {
        log.info(`Device not (yet) supported [ID]:${data.Id} [Type]:${data.Type} [EquipmentCode]:${data.EquipmentCode} [Name]:${data.Name}`)
        log.debug('Unsupported device found! [Data]:', JSON.stringify(data))
        return null
      }
  ...
  class Device {
    ...
    notify() {
      if (this.batteryService) {
        this.batteryService.updateCharacteristic(Characteristic.StatusLowBattery, this.getLowBatteryState())
  ...
  class ContactSensor extends Device {
    ...
    notify() {
      super.notify()
      if (this.service) {
        this.service.updateCharacteristic(Characteristic.ContactSensorState, this.getSensorState())
        this.service.updateCharacteristic(Characteristic.StatusTampered, this.getTamperedState())
      }
    }
  }
  ...
  class SmokeSensor extends Device {
    constructor(accessory) {
      super(accessory)
      this.service = accessory.getService(Service.SmokeSensor)
      this.batteryService.updateCharacteristic(Characteristic.ChargingState, Characteristic.ChargingState.NOT_CHARGEABLE)
    ...
    notify() {
      super.notify()
      if (this.service) {
        this.service.updateCharacteristic(Characteristic.SmokeDetected, this.getSensorState())
        this.service.updateCharacteristic(Characteristic.StatusTampered, this.getTamperedState())
      }
    }
  }
  ...
  class CarbonMonoxideSensor extends Device {
    constructor(accessory) {
      super(accessory)
      this.service = accessory.getService(Service.CarbonMonoxideSensor)
      this.batteryService = accessory.getService(Service.BatteryService)
    ...
    notify() {
      super.notify()
      if (this.service) {
        this.service.updateCharacteristic(Characteristic.CarbonMonoxideDetected, this.getSensorState())
        this.service.updateCharacteristic(Characteristic.StatusTampered, this.getTamperedState())
      }
    }
  }
  extend(CarbonMonoxideSensor, {
    appliesTo: (data) => {
      return (
        (data.Type == VivintDict.PanelDeviceType.WirelessSensor) && 
        (
  ...
  class Camera extends Device {
    ...
    async notify() {
      if (this.service) {
        ...
        if (motionDetected && config_cameraAutomationHttpPort !== null) {
          try {
            await request({
              method: "GET",
              url: `http://[::1]:${config_cameraAutomationHttpPort}/motion?${this.name}`
            })
          }
          catch (err) {
  ...
  class MotionSensor extends Device {
    ...
    notifyOccupancy() {
      if (this.occupancyService) {
        this.occupancyService
          .updateCharacteristic(Characteristic.OccupancyDetected, this.getOccupancyState())
      }
    }
  ...
  class LightSwitch extends Device {
    constructor(accessory) {
      ...
      this.service = accessory.getService(Service.Lightbulb) || accessory.getService(Service.Fan) || accessory.getService(Service.Switch)
  ...
  class Thermostat extends Device {
    constructor(accessory) {
      ...
      this.fanService = accessory.getService(Service.Fan)
