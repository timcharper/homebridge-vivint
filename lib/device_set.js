...
function DeviceSetModule(config, log, homebridge, vivintApi, ThermostatCharacteristics, setInterval, Date) {
  ...
  class Panel extends Device {
    ...
    notify(){
      //TODO: Check why this method is called twice for panel, second time with no state data
      let state = this.getCurrentSecuritySystemState()
      
      if (state !== undefined) {
        this.service
          .updateCharacteristic(Characteristic.SecuritySystemTargetState, this.getCurrentSecuritySystemState())
          .updateCharacteristic(Characteristic.SecuritySystemCurrentState, this.getCurrentSecuritySystemState())
      }
    }
  }
