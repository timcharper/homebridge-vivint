...
function DeviceSetModule(config, log, homebridge, vivintApi, ThermostatCharacteristics, setInterval, Date) {
  ...
  class DeviceSet {
    ...
    handleSnapshot(deviceData, timestamp) {
      ...
      for (let _deviceId in this.devicesById) {
        ...
        let data = deviceData.Data.find((dvc) => dvc.Id == deviceId)
  ...
  extend(DeviceSet, {
    createDeviceAccessory: (data) => {
      ...
      if (!deviceClass) {
        log("Do not know how to handle device! ID: " + data.Id + " Data.t: " + data.Type + " data.ec: " + data.EquipmentCode + " Data.n: " + data.Name)
        return null // we don't know how to handle this device.
      }
      if (config_IgnoredDeviceTypes.indexOf(data.Type) != -1){
        log("Ignored device ID: " + data.Id + " Data.t: " + data.Type + " data.ec: " + data.EquipmentCode + " Data.n: " + data.Name)
        return null
      }
    ...
    appliesTo: (data) => {
      return (
        ...
        (
          ...
          (data.EquipmentCode == VivintDict.EquipmentCode.EXISTING_DOOR_WINDOW_CONTACT) //|| (data.ec == 1248) || (data.ec == 2831) || (data.ec == 1269))
        ) 
      )
    },
    inferCategory: (data) => {
  ...
  extend(MotionSensor, {
    appliesTo: (data) => {
      return ((data.Type ==  VivintDict.PanelDeviceType.WirelessSensor) && (data.EquipmentCode == VivintDict.EquipmentCode.PIR2_MOTION))
    },
    addServices: (accessory) => {
