...
function DeviceSetModule(config, log, homebridge, vivintApi, ThermostatCharacteristics) {
  ...
  class Thermostat extends Device {
    ...
    getServices() {
      ...
      let getter = function(fn) {
        ...
        return (next) => {
          let result = bound()
          console.log("got value", fn.name, result)
          next(null, result)
        }
      }
      ...
      let promiseSetter = function(fn) {
        ...
        return (value, next) => {
          bound(value).then(
            (result) => {
              console.log("got success value", fn.name, result)
              next()
            },
            (failure) => {
              log("failure " + failure)
  ...
  // let Devices = [DoorWindowSensor, Lock, Thermostat] //NOCOMMIT
  let Devices = [Thermostat]
  return DeviceSet
}
