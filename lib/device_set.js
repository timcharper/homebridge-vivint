...
function DeviceSetModule(config, log, homebridge, vivintApi) {
  ...
  class DeviceSet {
    ...
    handleMessage(message) {
      ...
      if (message.Data) {
        ...
        else if (message.Type === VivintDict.ObjectType.InboxMessage && message.Data.Subject.indexOf('failed to lock') !== 0){
  ...
  extend(DeviceSet, {
    createDeviceAccessory: (data) => {
      ...
      if (!deviceClass) {
        log.warn(`Device not (yet) supported [ID]:${data.Id} [Type]:${data.Type} [EquipmentCode]:${data.EquipmentCode} [Name]:${data.Name}`)
        return null
      }
      if (config_IgnoredDeviceTypes.indexOf(data.Type) != -1){
        log.warn(`Ignored device [ID]:${data.Id} [Type]:${data.Type} [EquipmentCode]:${data.EquipmentCode} [Name]:${data.Name}`)
        return null
      }
    ...
    notify() {
      if (this.service) {
        this.service.getCharacteristic(Characteristic.MotionDetected)
          ...
          log.debug("Updated camera motion PIV: " + this.motionpivDetectedValue())
      }
    }
  }
  extend(Camera, {
  ...
  class Lock extends Device {
    ...
    async setTargetState(targetState, callback) {
      ...
      log('setTargetState', targetState)     
      try {
  ...
  class Panel extends Device {
    ...
    setTargetSecuritySystemState(targetState, callback) {
      ...
      vivintApi.putDevice('armedstates', null, {
        ...
        (failure) => {
          log.error("Failure setting panel state:", failure)
          callback(failure)
        })
    }
