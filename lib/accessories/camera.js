...
class Camera extends Device {
    constructor(accessory, data, config, log, homebridge, vivintApi) {
      ...
      this.rtspUrl = data.CameraInternalURL[0].replace('rtsp://',`rtsp://${vivintApi.panelLogin.Name}:${vivintApi.panelLogin.Password}@`) 
    ...
    async handleSnapshotRequest(request, callback) {
      ...
      let snapshot = [].concat(...snapshotArgs.map(arg => arg.map(a => typeof a == 'string' ? a.trim() : a)))

      try {
        let ffmpeg = spawn(this.ffmpegPath, [
          ...snapshot
        ], {
            env: process.env
        })
    ...
    async handleStreamRequest(request, callback) {
      ...
      switch(request.type) {
        ...
        case this.hap.StreamRequestTypes.RECONFIGURE:
          // not implemented
          this.log.debug('(Not implemented) Received request to reconfigure to: ' + JSON.stringify(request.video, undefined, 4))
          callback()
          break
      }
    }
    ...
    async notify() {
      ...
      if (this.controller && this.controller.doorbellService && this.getDoorbellButtonPress()){
